...
test("web search", async ({ browser, page: initialPage }) => {
  ...
  const clickCount = Number.parseInt((await page.$eval("[class='search-progress-current']", (el) => el.textContent)) || "0", 10);
  ...
  for (let idx = 0; idx < words.length; idx++) {
    ...
    await page.type("input[name='qt']", words[idx], {
      delay: randomNumber(100, 300),
    });
    await page.click("#searchBtn");
    console.info("Word: %s searched!", words[idx]);
    await page.waitForSelector("#myForm input[name='qt']", {
      state: "visible",
    });
