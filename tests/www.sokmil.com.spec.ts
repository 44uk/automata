...
test('login bonus', async ({ browser }) => {
  ...
  context.addCookies([
    { name: "AGEAUTH", value: "ok", domain: ".sokmil.com", path: "/" }
  ])
  ...
  if(await page.$('a.p-header-global__button--login')) {
    ...
    await page.type('input[name="id"]', SOKMIL_ID!, { delay: randomNumber(100, 300)})
    await page.type('input[name="pw"]', SOKMIL_PW!, { delay: randomNumber(100, 300)})
