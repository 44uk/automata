...
test('click point', async ({ browser }) => {
  ...
  if(await page.$('//div[@class="service-nav__login"]/*[.="前回のログイン"]')) {
    ...
    await page.type("form input[name='u']", RAKUTEN_ID!, { delay: randomNumber(100, 200) })
    await randomWait(page)
    await page.fill("form input[name='p']", '')
    await page.type("form input[name='p']", RAKUTEN_PW!, { delay: randomNumber(100, 200) })
  ...
  const xpath = "//*[@id='topArea']/div[contains(@class, 'topArea')]//img[contains(@alt, 'Check')]/parent::node()/parent::node()/parent::node()/div[@class='bnrBox']"
